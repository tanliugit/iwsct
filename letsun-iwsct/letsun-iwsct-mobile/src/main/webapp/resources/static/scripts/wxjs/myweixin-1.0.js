...
var wxJs = {
      ...
      */  
      ...
      getAppid:function(shopid,redirect){
	  	...
	  	/** 转向获取code页面 */
	  	toGetCode:function(shopid,isonly){
	  		var redirect_uri = window.location.href;  		
	  		this.getAppid(shopid, function(appid){
	  			//微信授权的固定url  scope=snsapi_userinfo|scope=snsapi_base 
	            var wx_oauth2_url = "https://open.weixin.qq.com/connect/oauth2/authorize";
	            ...
	            if(isonly){
	            	param = "?appid=" + appid + "&redirect_uri=" + encodeURIComponent(redirect_uri) + "&response_type=code&scope=snsapi_base&state=lxcy#wechat_redirect";
	            }
		...
		/** 只获取用户openid */
	  	getOnlyOpenidByCode:function(shopid,isonly, callback){
	      	var code = this.getUrlParam("code");
	          $.ajax({
	  			url:this.root + "/w/wxutil/getOpenidByCode",
	  			type:"POST",
	              data:{shopid:shopid, code:code,isonly:isonly, ts:(new Date()).getTime()},
	              dataType:"json",
	              success:function(data){
	            	callback(data);
	              },
 ...
 * 只获取openid 
 * */
function getOnlyOpenidForCorpid(_corpid,isonly,callback){
	var code = wxJs.getUrlParam("code");
	if(code == null || code == undefined){
		wxJs.toGetCode(_corpid,isonly);
		//return;
	}else{
		wxJs.getOnlyOpenidByCode(_corpid,isonly,function(data){
            callback(data);
		});
	}
}
...
//2异步获取微信jS-SDK 参数
function getJsApSign2(reurl,wxappid,corpid,getJsUrl){
	$.ajax({  
        type: 'GET',  
        url:getJsUrl+"/w/wxutil/getJsApSign2/"+corpid,
        dataType:'jsonp',  
        data:{  
				reurl:reurl,
				ts:(new Date()).getTime()
        ...
        success: function(data){
			...
			//提交成功
				...
				//设置微信参数
				var obj = new Function("return" + data.wxmaps)();//转换后的JSON对象
			    setwx_config(wxappid,obj.timestamp,obj.nonceStr,obj.signature);
			}
			//生成JS-SDK使用权限签名算法出错
			else if(data.status=='-1'){
				//alert("生成JS-SDK使用权限签名算法出错");
...
//微信分享
function wx_my_Share(reurl,shareData,corpid,wxappid){
	//var getJsUrl='http://zw.ebiaoji.com/wsct';   //'http://order.letsun.com.cn/';
	//var corpid = '${tcorp.corpid}';//  '91';//订阅号
	//var wxappid ='${tcorp.wxappid}'; //"wx02756e6ee0f837c1";
	//wxJs.setPrefix(getJsUrl);

	//异步获取微信jS-SDK 参数
	getJsApSign2(reurl,wxappid,corpid,wxJs.root);
